...
/*
                ...
                */
                ...
                int score = 255;
                for (int i = 0; i < K; ++i) {
                    if (rs[idx][i].w >= BG_MIN_WEIGHT) {       // mode établi, pas un réservoir tout neuf
                        established = true;
                        int d = std::max(std::max(
                                    abs((int)p.r - (int)rs[idx][i].rgbV.r),
                                    abs((int)p.g - (int)rs[idx][i].rgbV.g)),
                                    abs((int)p.b - (int)rs[idx][i].rgbV.b));
                        score = std::min(score, d);            // distance au mode le plus proche
                    }
                }
